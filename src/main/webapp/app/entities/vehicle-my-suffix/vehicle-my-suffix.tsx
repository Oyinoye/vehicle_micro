  ...
  return (
    <div>
      <h2 id="vehicle-my-suffix-heading" data-cy="VehicleHeading">
        <Translate contentKey="maxVehicleApp.vehicle.home.title">Vehicles</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="maxVehicleApp.vehicle.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="maxVehicleApp.vehicle.home.createLabel">Create new Vehicle</Translate>
          </Link>
        </div>
      </h2>
        ...
        {vehicleList && vehicleList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="maxVehicleApp.vehicle.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('vehicleID')}>
                  <Translate contentKey="maxVehicleApp.vehicle.vehicleID">Vehicle ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('plateNumber')}>
                  <Translate contentKey="maxVehicleApp.vehicle.plateNumber">Plate Number</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('age')}>
                  <Translate contentKey="maxVehicleApp.vehicle.age">Age</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th>
                  <Translate contentKey="maxVehicleApp.vehicle.vehicleLocation">Vehicle Location</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="maxVehicleApp.vehicle.manufacturer">Manufacturer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {vehicle.vehicleLocation ? (
                      <Link to={`vehicle-location-my-suffix/${vehicle.vehicleLocation.id}`}>{vehicle.vehicleLocation.code}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {vehicle.manufacturer ? (
                      <Link to={`manufacturer-my-suffix/${vehicle.manufacturer.id}`}>{vehicle.manufacturer.name}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="maxVehicleApp.vehicle.home.notFound">No Vehicles found</Translate>
            </div>
          )
        )}
      </div>
