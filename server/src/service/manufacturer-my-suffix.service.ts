...
export class ManufacturerService {
    ...
    constructor(
        @InjectRepository(ManufacturerRepository) private manufacturerEntityRepository: ManufacturerRepository,
    ) {}
    ...
    async findAndCount(options: FindManyOptions<ManufacturerDTO>): Promise<[ManufacturerDTO[], number]> {
        ...
        if (resultList && resultList[0]) {
            resultList[0].forEach(manufacturerEntity =>
                manufacturerEntityDTO.push(ManufacturerMapper.fromEntityToDTO(manufacturerEntity)),
            );
